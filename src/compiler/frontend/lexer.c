...
void new_lexer_error(struct lexer* lex, struct report* err)
{
    if(!lex || !err){
    ...
    if(!lex->errors){
        lex->errors = (struct report**)malloc(sizeof(struct report*));
        if(!lex->errors){
            free_report(err);
            return;
        }
    ...
    struct report** new_errors = (struct report**)realloc(
        lex->errors,
        (lex->errors_count + 1) * sizeof(struct report*)
    );
    if(!new_errors){
        free_report(err);
        return;
